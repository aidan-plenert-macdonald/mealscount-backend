...
def parse_districts(school_data, strategies, rates=None):
    ...
    for row in school_data:
        ...
        district_name, district_code = (
            row.get("District Name", row.get("district_name", "Default")),
            row.get("District Code", row.get("district_code", "1")),
        )
        if district_name not in districts:
            district = CEPDistrict(
                district_name, district_code, reimbursement_rates=rates
            )
            for s in strategies:
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Display grouping per district by school-code (must have target district specified)",
)
@click.option(
    "--show-schools",
    default=False,
    is_flag=True,
    help="Display individual school data (must have target district specified)",
)
@click.option(
    "--min-schools",
    default=None,
    help="If specified, only districts with at least N schools will be evaluated",
    type=int,
)
@click.option(
    "--list-strategies",
...
@click.option(
    "--output-json",
    default=None,
    help="If Specified, output will stored in filename specified in JSON format (defaults to output.json)",
)
@click.option(
    "--output-csv",
    default=None,
    help="If Specified with target district, output for first strategy will stored as CSV in filename",
)
@click.option(
    "--output-folder",
    default=None,
    help="Folder to output per-district json and district overview json for website",
)
@click.option(
    "--evaluate-by",
    default="reimbursement",
    help="Optimize by reimbursement or coverage",
)
@click.option(
    "--investigate",
    default=False,
    is_flag=True,
    help="Stop before exiting in a shell to investigate results",
)
@click.option(
    "--rates",
    default=None,
    help="Rates specified as freelunch,paidlunch,freebkfst,paidbkfst, e.g. '3.31,0.31,2.14,0.31' ",
)
...
def cli(
    ...
    schools = [
        r
        for r in csv.DictReader(codecs.open(cupc_csv_file))
        if r["total_enrolled"] and i(r["total_enrolled"]) > 0
    ]
    ...
    if target_district != None:
        schools = [
            s
            for s in schools
            if s.get("District Code", s.get("district_code", None)) == target_district
        ]
    ...
    print(
        "Districts with 10 or less schools: %0.0f%%"
        % (
            len([d for d in districts if len(d.schools) <= 10])
            / float(len(districts))
            * 100
        )
    )
    ...
    if strategies:
        ...
        click.secho(
            "{:,} Schools Processed for {:,} districts".format(
                n_schools, len(districts)
            ),
            fg="blue",
            bold=True,
        )

        baseline_eligible_overall = sum(
            [d.strategies[0].students_covered for d in districts]
        )
        best_eligible_overall = sum(
            [d.best_strategy.students_covered for d in districts]
        )
        improvement = best_eligible_overall - baseline_eligible_overall
        click.secho(
            "{:+,.0f} students eligible over baseline ({:,.0f} => {:,.0f}), {:+.0f}% change to ISP {:.0f}%".format(
                improvement,
    ...
    if output_folder:
        ...
        with open(fname, "w") as out_file:
            ...
            district_list = [
                d.as_dict(include_strategies=False, include_schools=False)
                for d in districts
            ]
