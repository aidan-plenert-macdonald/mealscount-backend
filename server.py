...
def get_district(state, code, district_params):
    ...
    if not os.path.exists(path):
        return None  # we may not have this state yet
    ...
    # Quick function to make "123,123" into an int 123123
    def i(x):
        return int(x.replace(",", ""))

    schools_data = [
        r
        for r in csv.DictReader(codecs.open(path))
        # ignore 0 student schools
        if i(r["total_enrolled"]) > 0 and r["District Code"] == code
    ]
    district = None
...
@app.route(
    '/api/districts/<regex("[a-z]{2}"):state>/<regex("[a-zA-Z0-9]+"):code>/',
    methods=["POST"],
)
